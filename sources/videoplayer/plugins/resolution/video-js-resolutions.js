...
videojs.plugin('resolutions', function(options) {
  ...
  videojs.each = function(obj, fn, context){
    for (var key in obj) {
  ...
  videojs.capitalize = function(string){
  ...
  this.resolutions_ = {
    ...
    bucketByTypes: function(sources){
      return videojs.reduce(sources, function(init, val, i){
    ...
    selectResolution: function(typeSources) {
      ...
      videojs.each(typeSources, function(i, s){
  ...
  player.changeResolution = function(new_source){
    ...
    this.ready(function() {
      ...
      videojs.setLocalStorage('videojs_preferred_res', parseInt(new_source.index, 10));
    });
  };
  ...
  var ResolutionMenuItem = videojs.MenuItem.extend({
    init: function(player, options){
      ...
      this.player_.one('loadstart', videojs.bind(this, this.update));
      this.player_.on('resolutionchange', videojs.bind(this, this.update));
    }
  });
  ...
  videojs.util.mergeOptions(player.controlBar.options['children'], {
    'resolutionsButton': {}
  });
  ...
  var source = player.resolutions_.selectSource(player.options().sources);
